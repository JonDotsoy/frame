...
const generatorInstance = <T>(deeps: Map<any, any[]>, instancesMap: Map<any, any>, classDeclaration: new (...args: any) => T): T => {
    ...
    if (!classDeclarationArguments) throw new FrameError(`Can't found class declaration ${classDeclaration.name}`)
    const args: any[] = classDeclarationArguments.map(classDeclarationArgument => generatorInstance(deeps, instancesMap, classDeclarationArgument))
